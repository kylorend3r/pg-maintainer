...
- [Replica-lag Gating](#replica-lag-gating)
- [Maintaining Specific Tables](#maintaining-specific-tables)
...
- **Replica-lag awareness**: `--max-replica-lag-seconds` holds maintenance on each table while any standby's `replay_lag` exceeds the threshold, waiting for it to recover and skipping the table if it does not. Off unless you set a threshold. See [Replica-lag gating](#replica-lag-gating).
- **Explicit table list**: `--also-tables public.orders,public.customers` runs `VACUUM (ANALYZE)` on named tables after the selected modes finish, in addition to them, with no discovery thresholds applied. See [Maintaining specific tables](#maintaining-specific-tables).
...
VACUUM generates a lot of WAL, so a maintenance run is one of the likelier reasons a
standby falls behind. Give pg-maintainer a threshold and it checks `pg_stat_replication`
before **each table**, waits for lag to recover, and skips the table if it does not:

```bash
# Hold each table while any standby is more than 30s behind
pg-maintainer -d mydb -s public --max-replica-lag-seconds 30

# Wait at most 60s per table instead of the default 300
pg-maintainer -d mydb -s public --max-replica-lag-seconds 30 --max-replica-lag-wait-seconds 60

# Never maintain while lagging, and never sleep waiting for it
pg-maintainer -d mydb -s public --max-replica-lag-seconds 30 --max-replica-lag-wait-seconds 0
```

The gate is **off unless you pass a threshold**. Nothing is queried and nothing changes
for existing runs.

### Choosing a threshold

There is no universal number, because the tolerance is a property of what the replica is
*for*. Healthy async replication on a LAN normally sits in the tens of milliseconds,
rising to a few hundred under write load.

| Replica's purpose | Suggested threshold |
|---|---|
| Serves user-facing reads | 5–10 seconds |
| HA failover target (lag is the data-loss window) | 30 seconds |
| Disaster recovery only | 300 seconds |

Measure before choosing: watch `replay_lag` over a busy period, take the 95th
percentile, and set the threshold at two to three times that with a floor around 10
seconds. Too low and vacuum's own WAL bursts trip the gate constantly, so maintenance
never finishes. Too high and it never fires when it matters.

### What it does and does not see

- **No replicas** disables the gate for the run, logged once. A single-primary instance
  is unaffected.
- **A NULL `replay_lag`** means the standby is caught up, and counts as zero.
- **Every row in `pg_stat_replication` counts**, including logical replication
  subscribers. A standby rebuilding in `catchup` state will show large lag and can cause
  every table to be skipped.
- **Reading the view needs `pg_read_all_stats`** (granted by `pg_monitor`). Without it
  the view is empty, which is indistinguishable from having no replicas, so the tool
  says lag is unobservable rather than pretending the cluster is healthy. Grant
  `pg_monitor` to the maintenance role to enable gating.
- **Replication slots with no active connection are invisible here**, so a slot holding
  back WAL is not detected.

A timed-out wait skips that table and moves on. With persistently high lag every
remaining table pays the full wait, so lower `--max-replica-lag-wait-seconds` if that
matters. The run summary reports total time waited and tables skipped. SIGTERM and
Ctrl-C interrupt a wait immediately rather than being noticed after it.

## Maintaining Specific Tables

`-t/--table` narrows discovery to one table, which must still clear its phase's criteria
to be touched. `--also-tables` is the opposite: it maintains the tables you name whether
or not they would ever be discovered.

```bash
# Normal run, plus these two tables regardless of thresholds
pg-maintainer -d mydb -s public --also-tables public.orders,public.customers

# Only the bloat phase, then these tables
pg-maintainer -d mydb -s public --mode bloated --also-tables public.orders
```

It runs as a final phase after whichever `--mode` values were selected, in addition to
them, issuing `VACUUM (VERBOSE, ANALYZE)` on each named table. It is not a `--mode`
value.

- **Names must be schema-qualified.** `public.orders`, never `orders`. Bare names are
  rejected before connecting, so a typo in a cron line fails immediately rather than
  silently doing nothing.
- **Names match the catalog exactly.** PostgreSQL folds unquoted identifiers to lower
  case at creation time, so `Public.Orders` will not match `public.orders`.
- **The schema must be one you are already maintaining.** The concurrency guard's
  advisory lock is derived from the resolved schema list, so a table outside it would be
  maintained without that protection. Add the schema to `--schema` to include it.
- **Missing tables warn and are skipped**, and the rest still run.
- **Tables already maintained by an earlier phase are skipped**, so nothing is vacuumed
  twice in one run.
- `--limit` and the size filters do not apply. Dry run, active-vacuum handling,
  `lock_timeout`, throttling and replica-lag gating all behave exactly as elsewhere.

One caveat worth knowing: PostgreSQL does not fail a VACUUM on a table the connected
role does not own. It emits a warning, skips the table, and reports success, so
pg-maintainer will log success for work it did not do. Make sure the maintenance role
owns the tables you name.
